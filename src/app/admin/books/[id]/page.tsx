...
export default function BookEditorPage() {
    ...
    return (
        <AdminLayout>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '40px', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '24px' }} className="fade-up">
                <Link href={`/admin/shelves/${book?.shelf.id}`} className="btn btn-ghost btn-sm" style={{ padding: '8px 20px' }}>← กลับไปที่หนังสือ</Link>
                <div>
                    <h1 className="text-gradient" style={{ fontSize: '2.2rem', fontWeight: 900 }}>แก้ไขหน้าหนังสือ: {book?.title}</h1>
                    <p style={{ fontSize: '0.9rem', color: 'var(--c-text-3)', marginTop: '4px' }}>🖼️ กำลังจัดการ {book?.pages.length || 0} หน้า ในชั้น "{book?.shelf.name}"</p>
                </div>
            </div>

            {/* Upload Section */}
            <div className="glass-panel mb-12 fade-up" style={{ padding: '48px', textAlign: 'center', animationDelay: '0.1s', border: '2px dashed rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)' }}>
                <div className="heart-pulse" style={{ fontSize: '4rem', marginBottom: '20px' }}>📸</div>
                <h2 style={{ fontSize: '1.4rem', marginBottom: '12px', fontWeight: 800 }}>อัปโหลดความทรงจำหน้าต่อๆ ไป (Add Pages)</h2>
                ...
                <input ref={fileInputRef} type="file" multiple accept="image/*" style={{ display: 'none' }} onChange={e => { if (e.target.files) uploadPages(e.target.files) }} />
            </div>
            ...
            {loading ? (
                ...
                <div className="glass-panel text-center fade-up" style={{ padding: '80px 40px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'grid', gap: '32px' }}>
                    {book?.pages.map((p, idx) => (
                        <div key={p.id} className="glass-panel fade-up" style={{ padding: '32px', display: 'flex', gap: '32px', flexWrap: 'wrap', animationDelay: `${idx * 0.08}s`, border: '1px solid rgba(255,255,255,0.05)' }}>
                            <div style={{ position: 'relative', width: '220px', height: '300px', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 12px 40px rgba(0,0,0,0.5)', background: '#000', flexShrink: 0, border: '1px solid rgba(255,255,255,0.1)' }}>
                                {p.imageUrl ? (
                                ...
                                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, transparent 60%, rgba(0,0,0,0.8) 100%)' }} />
                                <button
                            ...
                            <div style={{ flex: 1, minWidth: '340px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        <span className="badge" style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--c-text-3)' }}>หน้า {idx + 1}</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--c-text-3)', opacity: 0.5 }}>UID: {p.id}</span>
                                    </div>
            ...
            <div style={{ position: 'sticky', bottom: '32px', display: 'flex', justifyContent: 'center', pointerEvents: 'none', zIndex: 100 }}>
                <div className="glass-panel fade-up" style={{ padding: '16px 32px', pointerEvents: 'auto', display: 'flex', gap: '24px', alignItems: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.8)', border: '1px solid var(--c-pink)', animationDelay: '0.5s' }}>
                    <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 800, color: '#fff' }}>บันทึกอัตโนมัติ</div>
